...
export default function CompletedTasks() {
  ...
  useEffect(() => {
    ...
    // Fetch all tasks, then filter the ones with status "completed"
  ...
  if (loading)
    return (
      <p className="text-center mt-8">Loading completed tasks...</p>
    );
  if (error)
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="p-6 bg-white shadow-lg rounded-xl">
        {/* Header without the Create Task button */}
        <div className="flex items-center space-x-3 mb-6">
          <FontAwesomeIcon
            icon={faTasks}
            className="text-green-600 h-8 w-8 animate-pulse"
          />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {tasks.length > 0 ? (
            ...
            <p className="text-center text-gray-400">No completed tasks found.</p>
          )}
        </div>
      </div>
